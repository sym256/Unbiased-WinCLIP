...
def few_shot(memory, token, class_name, device):
    retrive = []
    for i in class_name:
        K, L, N, D = memory[i].shape # 2 169 1 640
        retrive.append(memory[i].permute(0, 3, 2, 1).reshape(K, D, -1)) # 2 640 169
    retrive = torch.stack(retrive, dim=1)# 2 32 640 169
     #B D L 
    true_memory = torch.zeros_like(retrive).detach().cpu() # 2 32 640 169
    for i, single_shot in enumerate(retrive): # 2, 32 640 169
        M = torch.min(1.0 - torch.bmm(F.normalize(token.squeeze(2), dim = -1), F.normalize(single_shot, dim = 1)), dim = -1)[1] # 32 169
        for j, m in enumerate(M): # 32, 169
            for k, sm in enumerate(m): # 169, 1
                true_memory[i, j, :, k] = retrive[i, j, :, sm] 
    
    K, B, C, HW = true_memory.shape
    mean = torch.mean(true_memory, dim=0).detach().cpu() # 32 640 169
    cov = torch.zeros(B, C, C, HW) # 32 640 640 169
    I = torch.eye(C)
    for b in range(B):
        for i in range(HW):
            cov[b][:, :, i] = torch.cov(true_memory[:, b, :, i].T) + 0.01 * I


    token = token.squeeze(2).permute(0,2,1).detach().cpu()
    N, C, HW = token.shape
    dist_list=[]
    for i in range(HW):
        dist = []
        for b, sample in enumerate(token):
            sample_mean = mean[b][:, i]
            conv_inv = torch.linalg.inv(cov[b][:, :, i])
            dist.append(mahalanobis_torch(sample[:, i], sample_mean, conv_inv))
        dist_list.append(dist)
    dist_list = torch.tensor(dist_list).transpose(1, 0).reshape(N, HW)
    return dist_list
...
def test(args,):
    ...
    for index, items  in enumerate(tqdm(test_dataloader)):
        ...
        # print("encoded image")
        # print("large_scale_tokens", large_scale_tokens.shape)
        # print("mid_scale_tokens", mid_scale_tokens.shape)
        # print("patch_tokens", patch_tokens.shape)
        # print("class_tokens", class_tokens.shape)
        # print("large_scale", large_scale.shape)
        # print("mid_scale", mid_scale.shape)
        # print("large_memory", large_memory['wood'].shape)
        # print("mid_memory", mid_memory['bottle'].shape)
        # print("patch_memory", patch_memory['wood'].shape)
        
        if few:
            m_l = few_shot(large_memory, large_scale_tokens, cls_name, device)
            m_m = few_shot(mid_memory, mid_scale_tokens, cls_name, device)
            m_p = few_shot(patch_memory, patch_tokens, cls_name, device)
            ...
            # print("harmonic aggregated few_shot")
            # print("m_l", m_l.shape)
            # print("m_m", m_m.shape)
            # print("m_p", m_p.shape)
...
if __name__ == '__main__':
    ...
    print(args.k_shot, args.dataset)
